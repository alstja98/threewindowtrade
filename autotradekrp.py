...
while True:
    try:
        ma30 = get_ma30("KRW-KRP")
        ma60 = get_ma60("KRW-KRP")
        ma120 = get_ma120("KRW-KRP")
        beforema120 = get_before_ma120("KRW-KRP")
        before_slowd = get_before_slowd("KRW-KRP")
        slowd = get_slowd("KRW-KRP")
        current_price = get_current_price("KRW-KRP")

        if  (beforema120 < ma120) and (slowd >= 25) and (before_slowd < 25) and\
            (ma60 < current_price) and (ma120 < current_price):
             krw = get_balance("KRW")
             if krw > 5000:
                 upbit.buy_market_order("KRW-KRP", krw*0.9995)
        elif slowd >= 75:
            doge = get_balance("KRP")
            if doge > 0.00008:
                upbit.sell_market_order("KRW-KRP", doge*0.9995)
