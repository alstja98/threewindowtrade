...
while True:
    try:
        ma30 = get_ma30("KRW-XRP")
        ma60 = get_ma60("KRW-XRP")
        ma120 = get_ma120("KRW-XRP")
        beforema120 = get_before_ma120("KRW-XRP")
        before_slowd = get_before_slowd("KRW-XRP")
        slowd = get_slowd("KRW-XRP")
        current_price = get_current_price("KRW-XRP")

        if  (beforema120 < ma120) and (slowd >= 25) and (before_slowd < 25) and\
            (ma60 < current_price) and (ma120 < current_price):
             krw = get_balance("KRW")
             doge = get_balance("XRP")
             if krw > 5000:
                 if doge > 0:
                     break
                 else:
            ...
            {(current_price < ma30) and (current_price < ma60) and (current_price < ma120)}:
            doge = get_balance("XRP")
            if doge > 0:
                upbit.sell_market_order("KRW-XRP", doge)
        time.sleep(1)
